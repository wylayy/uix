...
static const u8 font8x8[95][FONT_BYTES_PER_GLYPH] = {
    ...
    {0x00,0x0C,0x0C,0x00,0x00,0x0C,0x06,000}, /* U+003B (;) */
...
static u32 fg = 0xE8E8E8, bg = 0x101018;

static inline void fb_plot(u32 x, u32 y, u32 color)
{
    /* 32bpp only; other formats when a driver needs them */
    volatile u32 *base = (volatile u32 *)fb->address;
    base[y * (fb->pitch / 4) + x] = color;
}

static void fb_draw_char(char c, u32 cx, u32 cy)
{
    if (c < 0x20 || c > 0x7E)
        c = '?';
    ...
    for (u32 gy = 0; gy < 8; gy++) {
        u8 bits = glyph[gy];
        /* double-scan: each font row occupies 2 pixel rows (8x16 cell) */
        for (u32 rep = 0; rep < 2; rep++) {
            u32 py = cy * GLYPH_H + gy * 2 + rep;
            for (u32 gx = 0; gx < 8; gx++) {
                u32 px = cx * GLYPH_W + gx;
                u32 col = (bits & (1u << gx)) ? fg : bg;
                fb_plot(px, py, col);
            }
        }
    }
...
static void fb_scroll(void)
{
    ...
    for (u32 y = line; y < fb->height; y++)
        for (u32 x = 0; x < w; x++)
            base[(y - line) * w + x] = base[y * w + x];
    for (u32 y = fb->height - line; y < fb->height; y++)
        for (u32 x = 0; x < w; x++)
            base[y * w + x] = bg;
}
